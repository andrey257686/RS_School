import { ModelCarWinners } from "../../types/types";
import winnersListeners from "./winnersListeners";
import "./winers.scss";

export default class WinnersView {
  public winnersPage: HTMLDivElement;

  public winnersName: HTMLSpanElement;

  public winnersTable: HTMLTableElement;
  ...
  constructor() {
    ...
    this.winnersName = document.createElement("span");
  ...
  public createWinnersPage() {
    this.winnersPage.classList.add("winners");
    this.winnersPage.append(this.renderName());
    this.winnersPage.append(this.renderTable());
    this.winnersPage.append(this.renderPagination());
  }

  public renderName() {
    this.winnersName.classList.add("winners__name");
    return this.winnersName;
  }

  public renderTable() {
    this.winnersTable.classList.add("winners__table");
  ...
  public renderPagination() {
    this.winnersPagination.classList.add("winners__pagination");
    ...
    if (winnersListeners.handlePrevPageClick !== undefined) {
      buttonPrev.addEventListener("click", winnersListeners.handlePrevPageClick);
    }
    ...
    if (winnersListeners.handleNextPageClick !== undefined) {
      buttonNext.addEventListener("click", winnersListeners.handleNextPageClick);
    }
    buttonNext.textContent = "NEXT";
    this.winnersPagination.append(buttonPrev, currentPage, buttonNext);
    this.winnersPage.append(this.winnersPagination);
    return this.winnersPagination;
  }

  public updateWinnersPage(data: ModelCarWinners, page: number) {
    this.updateWinnersName(data);
    this.updateWinnersTable(data, page);
    this.updatePagination(page);
  }

  public updateWinnersName(data: ModelCarWinners) {
    const { count } = data;
    const element: HTMLSpanElement | null = document.querySelector(".winners__name");
    if (element !== null) {
      element.textContent = `Winners (${count})`;
    }
  }

  public updateWinnersTable(data: ModelCarWinners, page: number) {
    ...
    this.winnersTable.appendChild(trHead);
    carWinners.forEach((winner) => {
      ...
      this.winnersTable.appendChild(tr);
    });
    this.placeCounter = 1;
  }

  public updatePagination(page: number) {
    const element: HTMLSpanElement | null = document.querySelector(".winners__pagination_curPage");
    if (element !== null) {
  ...
  public renderCar(color: string) {
    ...
    element.classList.add("table__car");
